...
// SETTINGS
const apiKeySettingEl = document.getElementById('apiKeySetting');
const modelSettingEl = document.getElementById('modelSetting');
const promptSettingEl = document.getElementById('promptSetting');
const statusSettingEl = document.getElementById('statusSetting');
...
async function loadSettings() {
  const { openai_api_key, openai_model, openai_prompt } =
...
analyzeBtn.addEventListener('click', async () => {
  ...
  if (!key) {
    setStatus('Masukkan API key.');
    return;
  }
  if (!preview.src) {
    setStatus('Masukkan gambar dulu.');
    return;
  }
    ...
    const messages = [
      {
        role: 'system',
        content:
          'You are a concise vision assistant. Answer in Indonesian when the user prompt is Indonesian.',
      },
      {
        role: 'user',
        content: [
...
document.addEventListener('DOMContentLoaded', function () {
  const btn = document.getElementById('clearApiKeyBtn');
  const status = document.getElementById('clearApiKeyStatus');
  const apiKeyInput = document.getElementById('apiKey');
  btn.addEventListener('click', function () {
    if (
      confirm(
        'Are you sure you want to delete your API Key on Local Storage? This action cannot be undone.'
      )
    ) {
      // For Chrome extension: chrome.storage.local.remove('apiKey')
      if (
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const buttons = document.querySelectorAll('.tab[data-target]');
  const panels = document.querySelectorAll('.view');

  const imgPanel1 = document.querySelector('.prompt-icon');
  const imgPanel2 = document.querySelector('.settings-icon');

  buttons.forEach((button) => {
    const clickedTargetId = button.getAttribute('data-target');
    ...
    button.addEventListener('click', () => {
      ...
      buttons.forEach((btn) => {
        ...
        if (targetId === clickedTargetId) {
          ...
          imgPanel1.src = icons['panel-1'].inactive;
          imgPanel2.src = icons['panel-2'].active;
        } else {
          btn.classList.remove('tab-active');
          imgPanel1.src = icons['panel-1'].active;
          imgPanel2.src = icons['panel-2'].inactive;
        }
      });

      // Handle panel visibility
      panels.forEach((panel) => {
        if (panel.id === clickedTargetId) {
